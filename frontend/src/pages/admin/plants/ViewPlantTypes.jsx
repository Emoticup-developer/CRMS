...
import {
  ...
  updatePlantType,
  deletePlantType,
} from "../../../store/slices/plantTypeSlice";
import { FaTimes, FaTrashAlt } from "react-icons/fa";
import dayjs from "dayjs";
import { PiPlantLight } from "react-icons/pi";

const ViewPlantTypes = () => {
  const dispatch = useDispatch();
  const { plantTypes, loading } = useSelector((state) => state.plantTypes);
  ...
  const updateBreadcrumbs = (action = null, id = null) => {
    ...
    if (action === "Delete") {
      ...
      if (isBulkDelete && selectedPlantTypes.length > 0) {
        baseBreadcrumbs.push({
          label: formatIdsWithEllipsis(selectedPlantTypes),
          fullLabel: selectedPlantTypes.join(", "),
          path: null,
        });
      } else if (deleteId) {
        baseBreadcrumbs.push({
  ...
  useEffect(() => {
    ...
    } else if (showConfirm && selectedPlantType?.id) {
      updateBreadcrumbs("View", selectedPlantType.id);
    } else {
      updateBreadcrumbs();
    }
  }, [
  ...
  const handleOpenEdit = (plty) => {
    setEditId(plty.id);
    setFormData({
      name: plty.name || "",
      code: plty.code || "",
      short_name: plty.short_name || "",
      is_manufacturing:
        plty.is_manufacturing === null
          ? ""
          : plty.is_manufacturing
            ? "true"
            : "false",
      is_distribution:
        plty.is_distribution === null
          ? ""
          : plty.is_distribution
            ? "true"
            : "false",
      is_service:
        plty.is_service === null ? "" : plty.is_service ? "true" : "false",
      description: plty.description || "",
      is_active:
        plty.is_active === null ? "" : plty.is_active ? "true" : "false",
      sort_order: plty.sort_order || "",
    });
  ...
  const handleSubmitEdit = async (e) => {
    ...
    try {
      const res = await dispatch(
        updatePlantType({ id: editId, formData: data }),
      ).unwrap();

      if (res.status === 200 || res.status === 201) {
        showTemporaryMessage("Plant Type updated successfully!", "success");
      } else if (res.status === 202) {
        showTemporaryMessage("Plant Type update accepted!", "success");
      } else {
      ...
      if (errorMessages.length > 0) {
        ...
        showTemporaryMessage("Failed to update plant type!", "error");
      }
    }
  };
  ...
  const handleBulkDeleteClick = () => {
    if (selectedPlantTypes.length === 0) return;
    ...
    try {
      if (isBulkDelete && selectedPlantTypes.length > 0) {
        const res = await Promise.all(
          selectedPlantTypes.map((id) =>
            dispatch(deletePlantType(id)).unwrap(),
          ),
        );
        if (res.every((r) => r.status === 200 || r.status === 201)) {
          showTemporaryMessage("Plant Type deleted successfully!", "success");
        } else if (res.every((r) => r.status === 202)) {
          showTemporaryMessage("Plant Type delete accepted!", "success");
        } else {
          showTemporaryMessage("Unexpected response from server.", "error");
          return;
        }
        ...
        const res = await dispatch(deletePlantType(deleteId)).unwrap();
        if (res.status === 200 || res.status === 201) {
          showTemporaryMessage("Plant Type deleted successfully!", "success");
        } else if (res.status === 202) {
          showTemporaryMessage("Plant Type delete accepted!", "success");
      ...
      if (errorMessages.length > 0) {
        ...
        showTemporaryMessage("Failed to delete plant type!", "error");
      }
    }
  };

  // ---------------- FILTER LOGIC ----------------
  const filteredPlantTypes = plantTypes.filter((plty) => {
    const search = searchTerm.toLowerCase().trim().replace(/\s+/g, " ");
    const normalize = (value) =>
      (value || "").toString().toLowerCase().trim().replace(/\s+/g, " ");
    let activeText = "";
    if (plty?.is_active === true) activeText = "yes";
    else if (plty?.is_active === false) activeText = "no";
    return (
      normalize(plty?.name).includes(search) ||
      normalize(plty?.short_name).includes(search) ||
      normalize(plty?.creator?.username).includes(search) ||
      normalize(plty?.company?.company_name).includes(search) ||
      normalize(activeText).includes(search)
    );
  });

  // ---------------- PAGINATION LOGIC ----------------
  const totalPages = Math.ceil(filteredPlantTypes.length / rowsPerPage);

  const paginatedPlantTypes = filteredPlantTypes.slice(
        ...
        <div className="w-full mb-4">
          <div className="flex justify-between items-end border-b-2 border-gray-300 pb-1 mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <FiPlus /> Create Plant Type
              </Link>

              {selectedPlantTypes.length > 1 && (
                <button
                      ...
                      {filteredPlantTypes.length <= 1 ? (
                        ...
                        <input
                          ...
                          checked={
                            filteredPlantTypes.length > 1 &&
                            selectedPlantTypes.length ===
                              filteredPlantTypes.length
                          }
                          onChange={(e) =>
                            setSelectedPlantTypes(
                              e.target.checked
                                ? filteredPlantTypes.map((plty) => plty.id)
                                : [],
                            )
                          }
                    ...
                    paginatedPlantTypes.map((plty) => (
                      ...
                      >
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          <input
                            type="checkbox"
                            checked={selectedPlantTypes.includes(plty.id)}
                            onChange={() =>
                              setSelectedPlantTypes((prev) =>
                                prev.includes(plty.id)
                                  ? prev.filter((x) => x !== plty.id)
                                  : [...prev, plty.id],
                              )
                            }
                            className="w-4 h-4 cursor-pointer transition-all duration-200 ease-in-out transform hover:scale-110 active:scale-95 accent-amber-400"
                        ...
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          {plty?.creator?.username || "--"}
                        </td>
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          {plty?.company?.company_name || "--"}
                        </td>
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          {plty?.is_active == null ? (
                            "--"
                        ...
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          <div className="flex justify-center items-center space-x-3 text-sm">
                            ...
                            <button
                              onClick={() => {
                                setSelectedPlantType(plty);
                                updateBreadcrumbs("View", plty.id);
                                setShowConfirm(true);
                              }}
              ...
              <table className="table-auto w-full text-sm text-gray-700">
                <tbody>
                  ...
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedPlantType?.is_active == null ? (
                        "--"
                      ) : (
                        <span
                          className={
                            selectedPlantType?.is_active
                              ? "text-green-600"
                              : "text-red-500"
                          }
                        >
                          {selectedPlantType?.is_active ? "Yes" : "No"}
                        </span>
                      )}
                    </td>
                  </tr>
                  ...
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedPlantType?.is_manufacturing == null ? (
                        "--"
                      ) : (
                        <span
                          className={
                            selectedPlantType?.is_manufacturing
                              ? "text-green-600"
                              : "text-red-500"
                          }
                        >
                          {selectedPlantType?.is_manufacturing ? "Yes" : "No"}
                        </span>
                      )}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedPlantType?.is_distribution == null ? (
                        "--"
                      ) : (
                        <span
                          className={
                            selectedPlantType?.is_distribution
                              ? "text-green-600"
                              : "text-red-500"
                          }
                        >
                          {selectedPlantType?.is_distribution ? "Yes" : "No"}
                        </span>
                      )}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedPlantType?.is_service == null ? (
                        "--"
                      ) : (
                        <span
                          className={
                            selectedPlantType?.is_service
                              ? "text-green-600"
                              : "text-red-500"
                          }
                        >
                          {selectedPlantType?.is_service ? "Yes" : "No"}
                        </span>
                      )}
                    </td>
                  </tr>
                  ...
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedPlantType?.creator?.username || "--"}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    <td className="font-semibold w-2/5 py-1 text-left">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedPlantType?.company?.company_name || "--"}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    <td className="font-semibold w-2/5 py-1 text-left">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedPlantType?.created_at
                        ? dayjs(selectedPlantType?.created_at).format(
                            "DD-MM-YYYY hh:mm A",
                          )
                        : "--"}
                    </td>
                  ...
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedPlantType?.updated_at
                        ? dayjs(selectedPlantType?.updated_at).format(
                            "DD-MM-YYYY hh:mm A",
                          )
                        : "--"}
                    </td>
          ...
          <div className="bg-white p-4 rounded-md w-11/12 max-w-md shadow-xl">
            ...
            <div className="flex justify-center gap-4 mt-4">
              ...
              <button
                onClick={() => {
                  setShowDeleteModal(false);
                  if (isBulkDelete) setSelectedPlantTypes([]);
                  setIsBulkDelete(false);
                  setDeleteId(null);
                }}
