...
const ViewPosition = () => {
  ...
  const [selectedPositions, setSelectedPostions] = useState([]);
  ...
  const updateBreadcrumbs = (action = null, id = null) => {
    ...
    if (action === "Delete") {
      ...
      if (isBulkDelete && selectedPositions.length > 0) {
        baseBreadcrumbs.push({
          label: formatIdsWithEllipsis(selectedPositions),
          fullLabel: selectedPositions.join(", "),
          path: null,
        });
      } else if (deleteId) {
        baseBreadcrumbs.push({
  ...
  useEffect(() => {
    ...
    } else if (showConfirm && selectedPosition?.id) {
      updateBreadcrumbs("View", selectedPosition.id);
    } else {
      updateBreadcrumbs();
    }
  }, [
  ...
  const handleSubmitEdit = async (e) => {
    ...
    try {
      const res = await dispatch(
        updatePosition({ id: editId, formData: data }),
      ).unwrap();

      if (res.status === 200 || res.status === 201) {
        showTemporaryMessage("Position updated successfully!", "success");
      } else if (res.status === 202) {
        showTemporaryMessage("Position update accepted!", "success");
      } else {
      ...
      if (errorMessages.length > 0) {
        ...
        showTemporaryMessage("Failed to update Position!", "error");
      }
    }
  };
    ...
    try {
      if (isBulkDelete && selectedPositions.length > 0) {
        const res = await Promise.all(
          selectedPositions.map((id) => dispatch(deletePosition(id)).unwrap()),
        );
        if (res.every((r) => r.status === 200 || r.status === 201)) {
          showTemporaryMessage("Position deleted successfully!", "success");
        } else if (res.every((r) => r.status === 202)) {
          showTemporaryMessage("Position delete accepted!", "success");
        } else {
          showTemporaryMessage("Unexpected response from server.", "error");
          return;
        }
        ...
        const res = await dispatch(deletePosition(deleteId)).unwrap();
        if (res.status === 200 || res.status === 201) {
          showTemporaryMessage("Position deleted successfully!", "success");
        } else if (res.status === 202) {
          showTemporaryMessage("Position delete accepted!", "success");
      ...
      if (errorMessages.length > 0) {
        ...
        showTemporaryMessage("Failed to delete Position!", "error");
      }
    }
  };

  // ---------------- FILTER LOGIC ----------------
  const filteredPostions = positions.filter((position) => {
    const search = searchTerm.toLowerCase().trim().replace(/\s+/g, " ");
    const normalize = (value) =>
      (value || "").toString().toLowerCase().trim().replace(/\s+/g, " ");

    return (
      normalize(position?.position).includes(search) ||
      normalize(position?.position_code).includes(search) ||
      normalize(position?.position_description).includes(search)
    );
  });

  // ---------------- PAGINATION LOGIC ----------------
  const totalPages = Math.ceil(filteredPostions.length / rowsPerPage);

  const paginatedPositions = filteredPostions.slice(
                    ...
                    <th className="px-2 py-2 border border-gray-200 text-center sticky top-0 z-20">
                      {filteredPostions.length <= 1 ? (
                        ...
                        <input
                          ...
                          checked={
                            filteredPostions.length > 1 &&
                            selectedPositions.length === filteredPostions.length
                          }
                          onChange={(e) =>
                            setSelectedPostions(
                              e.target.checked
                                ? filteredPostions.map(
                      ...
                      >
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          <input
                            ...
                            onChange={() =>
                              setSelectedPostions((prev) =>
                                prev.includes(position.id)
                                  ? prev.filter((x) => x !== position.id)
                                  : [...prev, position.id],
                              )
                            }
                            className="w-4 h-4 cursor-pointer transition-all duration-200 ease-in-out transform hover:scale-110 active:scale-95 accent-amber-400"
                          ...
                          <div className="flex justify-center items-center space-x-3 text-sm">
                            ...
                            <button
                              onClick={() => {
                                ...
                                updateBreadcrumbs("View", position.id);
                                setShowConfirm(true);
                              }}
                  ...
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedPosition?.created_at
                        ? dayjs(selectedPosition?.created_at).format(
                            "DD-MM-YYYY hh:mm A",
                          )
                        : "--"}
                    </td>
                  ...
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedPosition?.updated_at
                        ? dayjs(selectedPosition?.updated_at).format(
                            "DD-MM-YYYY hh:mm A",
                          )
                        : "--"}
                    </td>
