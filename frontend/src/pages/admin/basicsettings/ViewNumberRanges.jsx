...
import {
  ...
  fetchModels,
  createNumberRange,
} from "../../../store/slices/numberRangesSlice";
import {
  closeConfirm,
  closeEditModal,
  openEditModal,
...
const ViewNumberRanges = () => {
  const dispatch = useDispatch();
  const { models, numberRanges, loading } = useSelector(
    (state) => state.numberRanges,
  );

  const { showConfirm, showEditModal, showDeleteModal } = useSelector(
    (state) => state.sidebar,
  );
  ...
  const handleOpenEdit = (nrrg) => {
    setEditId(nrrg.id);
    setFormData({
      model: nrrg.model?.id || "",
      suffix: nrrg.suffix || "",
      start_range: nrrg.start_range || "",
      end_range: nrrg.end_range || "",
    });
    ...
    try {
      if (editId) {
        const res = await dispatch(
          updateNumberRange({ id: editId, formData: data }),
        ).unwrap();
        if (res.status === 200 || res.status === 201) {
          showTemporaryMessage("Number Range updated successfully!", "success");
        } else if (res.status === 202) {
          showTemporaryMessage("Number Range update accepted!", "success");
        } else {
          showTemporaryMessage("Unexpected response from server.", "error");
          return;
        }
      } else {
        const res = await dispatch(createNumberRange(data)).unwrap();
        if (res.status === 200 || res.status === 201) {
          showTemporaryMessage("Number Range created successfully!", "success");
        } else if (res.status === 202) {
          showTemporaryMessage("Number Range create accepted!", "success");
        } else {
          showTemporaryMessage("Unexpected response from server.", "error");
          return;
        }
      ...
      if (errorMessages.length > 0) {
        ...
        showTemporaryMessage(
          "Failed to update or create number range!",
          "error",
        );
      }
    }
  ...
  const handleBulkDeleteClick = () => {
    if (selectedNumberRanges.length === 0) return;
    ...
    try {
      if (isBulkDelete && selectedNumberRanges.length > 0) {
        const res = await Promise.all(
          selectedNumberRanges.map((id) =>
            dispatch(deleteNumberRange(id)).unwrap(),
          ),
        );
        if (res.every((r) => r.status === 200 || r.status === 201)) {
          showTemporaryMessage("Number Range deleted successfully!", "success");
        } else if (res.every((r) => r.status === 202)) {
          showTemporaryMessage("Number Range delete accepted!", "success");
        } else {
          showTemporaryMessage("Unexpected response from server.", "error");
          return;
        }
        ...
        const res = await dispatch(deleteNumberRange(deleteId)).unwrap();
        if (res.status === 200 || res.status === 201) {
          showTemporaryMessage("Number Range deleted successfully!", "success");
        } else if (res.status === 202) {
          showTemporaryMessage("Number Range delete accepted!", "success");
      ...
      if (errorMessages.length > 0) {
        ...
        showTemporaryMessage("Failed to delete number range!", "error");
      }
    }
  };

  // ---------------- FILTER LOGIC ----------------
  const filteredNumberRanges = numberRanges.filter((nrrg) => {
    const search = searchTerm.toLowerCase().trim().replace(/\s+/g, " ");
    const normalize = (value) =>
      (value || "").toString().toLowerCase().trim().replace(/\s+/g, " ");

    return (
      normalize(nrrg?.model?.name).includes(search) ||
      normalize(nrrg?.suffix).includes(search) ||
      normalize(nrrg?.start_range).includes(search) ||
      normalize(nrrg?.end_range).includes(search)
    );
  });

  // ---------------- PAGINATION LOGIC ----------------
  const totalPages = Math.ceil(filteredNumberRanges.length / rowsPerPage);

  const paginatedNumberRanges = filteredNumberRanges.slice(
  ...
  return (
    <div className="flex h-screen">
      ...
      <main className="flex-1 ml-0 lg:ml-[325px] mt-[148px] p-3 overflow-y-auto bg-white backdrop-blur-sm shadow-inner [&::-webkit-scrollbar]:hidden scrollbar-none">
        ...
        <div className="w-full mb-4">
          <div className="flex justify-between items-end border-b-2 border-gray-300 pb-1 mb-4">
            <div className="flex items-center gap-2">
              <MdFormatListNumbered className="text-amber-500 text-lg" />
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <FiPlus /> Create Number Range
              </button>

              {selectedNumberRanges.length > 1 && (
                <button
              ...
              <table className="min-w-full text-sm text-left divide-y divide-gray-200">
                      ...
                      {filteredNumberRanges.length <= 1 ? (
                        ...
                        <input
                          ...
                          checked={
                            filteredNumberRanges.length > 1 &&
                            selectedNumberRanges.length ===
                              filteredNumberRanges.length
                          }
                          onChange={(e) =>
                            setSelectedNumberRanges(
                              e.target.checked
                                ? filteredNumberRanges.map((nrrg) => nrrg.id)
                                : [],
                            )
                          }
                ...
                <tbody className="divide-y divide-gray-100 text-gray-800">
                  {loading ? (
                    ...
                    paginatedNumberRanges.map((nrrg) => (
                      <tr
                        key={nrrg.id}
                      ...
                      >
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          <input
                            type="checkbox"
                            checked={selectedNumberRanges.includes(nrrg.id)}
                            onChange={() =>
                              setSelectedNumberRanges((prev) =>
                                prev.includes(nrrg.id)
                                  ? prev.filter((x) => x !== nrrg.id)
                                  : [...prev, nrrg.id],
                              )
                            }
                            className="w-4 h-4 cursor-pointer transition-all duration-200 ease-in-out transform hover:scale-110 active:scale-95 accent-amber-400"
                        ...
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          <div className="flex justify-center items-center space-x-3 text-sm">
                            ...
                            <button
                              onClick={() => {
                                setSelectedNumberRange(nrrg);
                                updateBreadcrumbs("View", nrrg.id);
        ...
        {showConfirm && selectedNumberRange && (
                  ...
                  <tbody>
                    <tr className="border-b border-gray-200">
                      ...
                      <td className="w-3/5 py-1 text-left pl-4">
                        {selectedNumberRange?.model?.model_name || "--"}
                      </td>
                    </tr>
                    <tr className="border-b border-gray-200">
                      <td className="font-semibold w-2/5 py-1 text-left">
                    ...
                    <tr className="border-b border-gray-200">
                      ...
                      <td className="w-3/5 py-1 text-left pl-4">
                        {selectedNumberRange?.start_range || "--"}
                      </td>
                    </tr>
                    <tr className="border-b border-gray-200">
                      <td className="font-semibold w-2/5 py-1 text-left">
                    ...
                    <tr className="border-b border-gray-200">
                      ...
                      <td className="w-3/5 py-1 text-left pl-4">
                        {selectedNumberRange?.created_at
                          ? dayjs(selectedNumberRange?.created_at).format(
                              "DD-MM-YYYY hh:mm A",
                            )
                          : "--"}
                      </td>
                    ...
                    <tr className="border-b border-gray-200">
                      ...
                      <td className="w-3/5 py-1 text-left pl-4">
                        {selectedNumberRange?.updated_at
                          ? dayjs(selectedNumberRange?.updated_at).format(
                              "DD-MM-YYYY hh:mm A",
                            )
                          : "--"}
                      </td>
            ...
            <div className="bg-white pt-0 pb-6 pl-6 pr-6 rounded-md w-11/12 max-w-md border border-gray-300">
              <div className="flex justify-between items-center border-b-2 pb-2 mt-4 mb-4 border-gray-300">
                <div className="flex items-center gap-2">
                  <MdFormatListNumbered className="text-amber-500 text-lg" />
                  <h2 className="text-lg font-semibold text-gray-700">
                    {editId ? "Change" : "Create"} Number Range
                  </h2>
                </div>
                <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-gray-700 max-h-[200px] overflow-y-auto [&::-webkit-scrollbar]:hidden scrollbar-none">
                ...
                <div className="flex flex-col">
                  <label className="form-label">
                    Start Range <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="number"
                    name="start_range"
                ...
                <div className="flex flex-col">
                  <label className="form-label">
                    End Range <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="number"
                    name="end_range"
            ...
            <div className="bg-white px-6 py-4 rounded-md w-11/12 max-w-sm shadow-md border border-gray-300">
              ...
              <div className="flex justify-center gap-4 mt-2">
                ...
                <button
                  onClick={() => {
                    dispatch(closeDeleteModal());
                    if (isBulkDelete) setSelectedNumberRanges([]);
                    setIsBulkDelete(false);
                    setDeleteId(null);
                  }}
...
export default ViewNumberRanges;
