...
const ViewUserStatus = () => {
  ...
  const { userstatuses, loading } = useSelector((state) => state.userstatuses);
  ...
  const updateBreadcrumbs = (action = null, id = null) => {
    ...
    if (action === "Delete") {
      ...
      if (isBulkDelete && selectedUserstatuses.length > 0) {
        baseBreadcrumbs.push({
          label: formatIdsWithEllipsis(selectedUserstatuses),
          fullLabel: selectedUserstatuses.join(", "),
          path: null,
        });
      } else if (deleteId) {
        baseBreadcrumbs.push({
  ...
  useEffect(() => {
    ...
    } else if (showConfirm && selectedUserstatus?.id) {
      updateBreadcrumbs("View", selectedUserstatus.id);
    } else {
      updateBreadcrumbs();
    }
  }, [
  ...
  const handleOpenEdit = (userstatus) => {
    setEditId(userstatus.id);
    setFormData({
      status: userstatus.status || "",
      status_code: userstatus.status_code || "",
      status_description: userstatus.status_description || "",
    });
  ...
  const handleSubmitEdit = async (e) => {
    ...
    try {
      const res = await dispatch(
        updateUserstatus({ id: editId, formData: data }),
      ).unwrap();

      if (res.status === 200 || res.status === 201) {
        showTemporaryMessage("User Status updated successfully!", "success");
      } else if (res.status === 202) {
        showTemporaryMessage("User Status update accepted!", "success");
      } else {
      ...
      if (errorMessages.length > 0) {
        ...
        showTemporaryMessage("Failed to update User Status!", "error");
      }
    }
  };
  ...
  const handleBulkDeleteClick = () => {
    if (selectedUserstatuses.length === 0) return;
    ...
    try {
      if (isBulkDelete && selectedUserstatuses.length > 0) {
        const res = await Promise.all(
          selectedUserstatuses.map((id) =>
            dispatch(deleteUserstatus(id)).unwrap(),
          ),
        );
        if (res.every((r) => r.status === 200 || r.status === 201)) {
          showTemporaryMessage("User Status deleted successfully!", "success");
        } else if (res.every((r) => r.status === 202)) {
          showTemporaryMessage("User Status delete accepted!", "success");
        } else {
          showTemporaryMessage("Unexpected response from server.", "error");
          return;
        }
        ...
        const res = await dispatch(deleteUserstatus(deleteId)).unwrap();
        if (res.status === 200 || res.status === 201) {
          showTemporaryMessage("User Status deleted successfully!", "success");
        } else if (res.status === 202) {
          showTemporaryMessage("User Status delete accepted!", "success");
      ...
      if (errorMessages.length > 0) {
        ...
        showTemporaryMessage("Failed to delete User Status!", "error");
      }
    }
  };

  // ---------------- FILTER LOGIC ----------------
  const filteredUserstatuses = userstatuses.filter((userstatus) => {
    const search = searchTerm.toLowerCase().trim().replace(/\s+/g, " ");
    const normalize = (value) =>
      (value || "").toString().toLowerCase().trim().replace(/\s+/g, " ");

    return (
      normalize(userstatus?.status).includes(search) ||
      normalize(userstatus?.status_code).includes(search) ||
      normalize(userstatus?.status_description).includes(search) ||
      normalize(userstatus?.creator?.username).includes(search) ||
      normalize(userstatus?.company?.company_name).includes(search)
    );
  });

  // ---------------- PAGINATION LOGIC ----------------
  const totalPages = Math.ceil(filteredUserstatuses.length / rowsPerPage);

  const paginatedUserstatuses = filteredUserstatuses.slice(
  ...
  return (
    <div className="flex h-screen">
        ...
        <div className="w-full mb-4">
          <div className="flex justify-between items-end border-b-2 border-gray-300 pb-1 mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <FiPlus /> Create User Status
              </Link>

              {selectedUserstatuses.length > 1 && (
                <button
              ...
              <table className="min-w-full text-sm text-left divide-y divide-gray-200">
                      ...
                      {filteredUserstatuses.length <= 1 ? (
                        ...
                        <input
                          ...
                          checked={
                            filteredUserstatuses.length > 1 &&
                            selectedUserstatuses.length ===
                              filteredUserstatuses.length
                          }
                          onChange={(e) =>
                            setSelectedUserstatuses(
                              e.target.checked
                                ? filteredUserstatuses.map(
                ...
                <tbody className="divide-y divide-gray-100 text-gray-800">
                  {loading ? (
                    ...
                    paginatedUserstatuses.map((userstatus) => (
                      <tr
                        key={userstatus.id}
                      ...
                      >
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          <input
                            type="checkbox"
                            checked={selectedUserstatuses.includes(
                              userstatus.id,
                            )}
                            onChange={() =>
                              setSelectedUserstatuses((prev) =>
                                prev.includes(userstatus.id)
                                  ? prev.filter((x) => x !== userstatus.id)
                                  : [...prev, userstatus.id],
                              )
                            }
                            className="w-4 h-4 cursor-pointer transition-all duration-200 ease-in-out transform hover:scale-110 active:scale-95 accent-amber-400"
                        ...
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          {userstatus?.status_description || "--"}
                        </td>
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          {userstatus?.creator?.username || "--"}
                        </td>
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          {userstatus?.company?.company_name || "--"}
                        </td>

                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          <div className="flex justify-center items-center space-x-3 text-sm">
                            <button
                              onClick={() => handleOpenEdit(userstatus)}
                            ...
                            <button
                              onClick={() => {
                                ...
                                updateBreadcrumbs("View", userstatus.id);
                                setShowConfirm(true);
                              }}
      ...
      {showConfirm && selectedUserstatus && (
              ...
              <table className="table-auto w-full text-sm text-gray-700">
                <tbody>
                  ...
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedUserstatus?.status_code || "--"}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4 align-top">
                      {selectedUserstatus?.status_description || "--"}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    <td className="font-semibold w-2/5 py-1 text-left">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedUserstatus?.creator?.username || "--"}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    <td className="font-semibold w-2/5 py-1 text-left">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedUserstatus?.company?.company_name || "--"}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    <td className="font-semibold w-2/5 py-1 text-left">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedUserstatus?.created_at
                        ? dayjs(selectedUserstatus?.created_at).format(
                            "DD-MM-YYYY hh:mm A",
                          )
                        : "--"}
                    </td>
                  ...
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedUserstatus?.updated_at
                        ? dayjs(selectedUserstatus?.updated_at).format(
                            "DD-MM-YYYY hh:mm A",
                          )
                        : "--"}
                    </td>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-gray-700 max-h-[200px] overflow-y-auto [&::-webkit-scrollbar]:hidden scrollbar-none">
              ...
              <div className="flex flex-col col-span-2">
                ...
                <textarea
                  ...
                  placeholder="Enter User status description..."
                ></textarea>
              </div>
            </div>
          ...
          <div className="bg-white p-4 rounded-md w-11/12 max-w-md shadow-xl">
            ...
            <div className="flex justify-center gap-4 mt-4">
              ...
              <button
                onClick={() => {
                  setShowDeleteModal(false);
                  if (isBulkDelete) setSelectedUserstatuses([]);
                  setIsBulkDelete(false);
                  setDeleteId(null);
                }}
