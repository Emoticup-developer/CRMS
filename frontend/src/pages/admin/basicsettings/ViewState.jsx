...
const ViewState = () => {
  ...
  const updateBreadcrumbs = (action = null, id = null) => {
    ...
    if (action === "Delete") {
      ...
      if (isBulkDelete && selectedStates.length > 0) {
        baseBreadcrumbs.push({
          label: formatIdsWithEllipsis(selectedStates),
          fullLabel: selectedStates.join(", "),
          path: null,
        });
      } else if (deleteId) {
        baseBreadcrumbs.push({
  ...
  useEffect(() => {
    ...
    } else if (showConfirm && selectedState?.id) {
      updateBreadcrumbs("View", selectedState.id);
    } else {
      updateBreadcrumbs();
    }
  }, [
  ...
  const handleSubmitEdit = async (e) => {
    ...
    Object.keys(formData).forEach((key) => {
      if (key === "state_logo") {
        if (formData.state_logo instanceof File) {
          data.append("state_logo", formData.state_logo);
        }
      } else {
    ...
    try {
      const res = await dispatch(
        updateState({ id: editId, formData: data }),
      ).unwrap();

      if (res.status === 200 || res.status === 201) {
        showTemporaryMessage("State updated successfully!", "success");
      } else if (res.status === 202) {
        showTemporaryMessage("State update accepted!", "success");
      } else {
      ...
      if (errorMessages.length > 0) {
        ...
        showTemporaryMessage("Failed to update State!", "error");
      }
    }
  };
    ...
    try {
      if (isBulkDelete && selectedStates.length > 0) {
        const res = await Promise.all(
          selectedStates.map((id) => dispatch(deleteState(id)).unwrap()),
        );
        if (res.every((r) => r.status === 200 || r.status === 201)) {
          showTemporaryMessage("State deleted successfully!", "success");
        } else if (res.every((r) => r.status === 202)) {
          showTemporaryMessage("State delete accepted!", "success");
        } else {
          showTemporaryMessage("Unexpected response from server.", "error");
          return;
        }
        ...
        const res = await dispatch(deleteState(deleteId)).unwrap();
        if (res.status === 200 || res.status === 201) {
          showTemporaryMessage("State deleted successfully!", "success");
        } else if (res.status === 202) {
          showTemporaryMessage("State delete accepted!", "success");
      ...
      if (errorMessages.length > 0) {
        ...
        showTemporaryMessage("Failed to delete State!", "error");
      }
    }
  };
  ...
  const filteredStates = states.filter((state) => {
    ...
    return (
      normalize(state?.state_name).includes(search) ||
      normalize(state?.state_code).includes(search) ||
      normalize(state?.state_country?.country_name).includes(search) ||
      normalize(state?.creator?.username).includes(search) ||
      normalize(state?.company?.company_name).includes(search)
    );
  });
        ...
        <div className="w-full mb-4">
          <div className="flex justify-between items-end border-b-2 border-gray-300 pb-1 mb-4">
            <div className="flex items-center gap-2">
              <FiFlag className="text-amber-500 text-lg" />
                      ...
                      {filteredStates.length <= 1 ? (
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setSelectedStates(
                              e.target.checked
                                ? filteredStates.map((state) => state.id)
                                : [],
                            )
                          }
                  ...
                  {loading ? (
                    ...
                    paginatedStates.map((state) => (
                      ...
                      >
                        ...
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          {state?.state_country?.country_name || "--"}
                        </td>
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          ...
                          <div className="flex justify-center items-center space-x-3 text-sm">
                            ...
                            <button
                              onClick={() => {
                                ...
                                updateBreadcrumbs("View", state.id);
                                setShowConfirm(true);
                              }}
              ...
              <table className="table-auto w-full text-sm text-gray-700">
                <tbody>
                  ...
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedState.state_logo && (
                        <a
                          href={
                            selectedState.state_logo.startsWith("http")
                              ? selectedState.state_logo
                              : `${api.defaults.baseURL.replace(/\/$/, "")}${selectedState.state_logo}`
                          }
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-amber-500"
                        >
                          {selectedState.state_logo.split("/").pop()}
                        </a>
                      )}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedState?.state_country?.country_name || "--"}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    <td className="font-semibold w-2/5 py-1 text-left">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedState?.creator?.username || "--"}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    <td className="font-semibold w-2/5 py-1 text-left">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedState?.company?.company_name || "--"}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    <td className="font-semibold w-2/5 py-1 text-left">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedState?.created_at
                        ? dayjs(selectedState?.created_at).format(
                            "DD-MM-YYYY hh:mm A",
                          )
                        : "--"}
                    </td>
                  ...
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedState?.updated_at
                        ? dayjs(selectedState?.updated_at).format(
                            "DD-MM-YYYY hh:mm A",
                          )
                        : "--"}
                    </td>
          ...
          <div className="bg-white pt-0 pb-6 pl-6 pr-6 rounded-md w-11/12 max-w-md border border-gray-300">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-gray-700 max-h-[200px] overflow-y-auto [&::-webkit-scrollbar]:hidden scrollbar-none">
              ...
              <div className="flex flex-col col-span-2">
                <label className="form-label">
                  State Logo <span className="text-red-500">*</span>
                </label>
                <input
                  type="file"
                  name="state_logo"
                  ...
                  className="form-input"
                />
                {existingFiles.state_logo && (
                  <a
                    href={
                      existingFiles.state_logo.startsWith("http")
                        ? existingFiles.state_logo
                        : `${api.defaults.baseURL.replace(/\/$/, "")}${existingFiles.state_logo}`
                    }
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-amber-500"
                  >
                    {existingFiles.state_logo.split("/").pop()}
                  </a>
                )}
              </div>
              <div className="flex flex-col">
          ...
          <div className="bg-white p-4 rounded-md w-11/12 max-w-md shadow-xl">
            ...
            <div className="flex justify-center gap-4 mt-4">
              ...
              <button
                onClick={() => {
                  setShowDeleteModal(false);
                  if (isBulkDelete) setSelectedStates([]);
                  setIsBulkDelete(false);
                  setDeleteId(null);
                }}
