...
import { createPosition } from "../../../store/slices/positionSlice";
...
const CreatePosition = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await dispatch(createPosition(submitData)).unwrap();
      if (res.status === 200 || res.status === 201) {
        showTemporaryMessage("Position  created successfully!", "success");
      } else if (res.status === 202) {
        showTemporaryMessage("Position  create accepted!", "success");
      } else {
      ...
      if (errorMessages.length > 0) {
        ...
        showTemporaryMessage("Failed to create Position!", "error");
      }
    }
