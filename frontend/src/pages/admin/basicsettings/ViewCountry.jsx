...
const ViewCountry = () => {
  ...
  const updateBreadcrumbs = (action = null, id = null) => {
    ...
    if (action === "Delete") {
      ...
      if (isBulkDelete && selectedCountries.length > 0) {
        baseBreadcrumbs.push({
          label: formatIdsWithEllipsis(selectedCountries),
          fullLabel: selectedCountries.join(", "),
          path: null,
        });
      } else if (deleteId) {
        baseBreadcrumbs.push({
  ...
  useEffect(() => {
    ...
    } else if (showConfirm && selectedCountry?.id) {
      updateBreadcrumbs("View", selectedCountry.id);
    } else {
      updateBreadcrumbs();
    }
  }, [
  ...
  const handleSubmitEdit = async (e) => {
    ...
    try {
      const res = await dispatch(
        updateCountry({ id: editId, formData: data }),
      ).unwrap();

      if (res.status === 200 || res.status === 201) {
        showTemporaryMessage("Country updated successfully!", "success");
      } else if (res.status === 202) {
        showTemporaryMessage("Country update accepted!", "success");
      } else {
      ...
      if (errorMessages.length > 0) {
        ...
        showTemporaryMessage("Failed to update Country!", "error");
      }
    }
  };
    ...
    try {
      if (isBulkDelete && selectedCountries.length > 0) {
        const res = await Promise.all(
          selectedCountries.map((id) => dispatch(deleteCurrency(id)).unwrap()),
        );
        if (res.every((r) => r.status === 200 || r.status === 201)) {
          showTemporaryMessage("Country deleted successfully!", "success");
        } else if (res.every((r) => r.status === 202)) {
          showTemporaryMessage("Country delete accepted!", "success");
        } else {
          showTemporaryMessage("Unexpected response from server.", "error");
          return;
        }
        ...
        const res = await dispatch(deleteCountry(deleteId)).unwrap();
        if (res.status === 200 || res.status === 201) {
          showTemporaryMessage("Country deleted successfully!", "success");
        } else if (res.status === 202) {
          showTemporaryMessage("Country delete accepted!", "success");
      ...
      if (errorMessages.length > 0) {
        ...
        showTemporaryMessage("Failed to delete Country!", "error");
      }
    }
  };
  ...
  const filteredCountries = countries.filter((country) => {
    ...
    return (
      normalize(country?.country_code).includes(search) ||
      normalize(country?.country_name).includes(search) ||
      normalize(country?.country_mobile).includes(search) ||
      normalize(country?.creator?.username).includes(search) ||
      normalize(country?.company?.company_name).includes(search)
    );
  });
        ...
        <div className="w-full mb-4">
          <div className="flex justify-between items-end border-b-2 border-gray-300 pb-1 mb-4">
            <div className="flex items-center gap-2">
              <FiGlobe className="text-amber-500 text-lg" />
                      ...
                      {filteredCountries.length <= 1 ? (
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setSelectedCountries(
                              e.target.checked
                                ? filteredCountries.map((country) => country.id)
                                : [],
                            )
                          }
                  ...
                  {loading ? (
                    ...
                    paginatedCountries.map((country) => (
                      ...
                      >
                        ...
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          {country?.country_mobile || "--"}
                        </td>
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          {country?.creator?.username || "--"}
                        </td>
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          {country?.company?.company_name || "--"}
                        </td>
                          ...
                          <div className="flex justify-center items-center space-x-3 text-sm">
                            ...
                            <button
                              onClick={() => {
                                ...
                                updateBreadcrumbs("View", country.id);
                                setShowConfirm(true);
                              }}
              ...
              <table className="table-auto w-full text-sm text-gray-700">
                <tbody>
                  ...
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedCountry?.country_mobile || "--"}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    <td className="font-semibold w-2/5 py-1 text-left">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedCountry.country_logo && (
                        <a
                          href={
                            selectedCountry.country_logo.startsWith("http")
                              ? selectedCountry.country_logo
                              : `${api.defaults.baseURL.replace(/\/$/, "")}${selectedCountry.country_logo}`
                          }
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-amber-500"
                        >
                          {selectedCountry.country_logo.split("/").pop()}
                        </a>
                      )}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedCountry?.creator?.username || "--"}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    <td className="font-semibold w-2/5 py-1 text-left">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedCountry?.company?.company_name || "--"}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    <td className="font-semibold w-2/5 py-1 text-left">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedCountry?.created_at
                        ? dayjs(selectedCountry?.created_at).format(
                            "DD-MM-YYYY hh:mm A",
                          )
                        : "--"}
                    </td>
                  ...
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedCountry?.updated_at
                        ? dayjs(selectedCountry?.updated_at).format(
                            "DD-MM-YYYY hh:mm A",
                          )
                        : "--"}
                    </td>
        ...
        <div className="fixed inset-0 backdrop-blur-[1px] flex justify-center items-center z-50">
          <div className="bg-white pt-0 pb-6 pl-6 pr-6 rounded-md w-11/12 max-w-md border border-gray-300">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-gray-700 max-h-[200px] overflow-y-auto [&::-webkit-scrollbar]:hidden scrollbar-none">
              ...
              <div className="flex flex-col">
                <label className="form-label">
                  Country Mobile <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  name="country_mobile"
                  placeholder="Enter Country Moblie"
              ...
              <div className="flex flex-col col-span-2">
                ...
                <input
                  ...
                  className="form-input"
                />
                {existingFiles.country_logo && (
                  <a
                    href={
                      existingFiles.country_logo.startsWith("http")
                        ? existingFiles.country_logo
                        : `${api.defaults.baseURL.replace(/\/$/, "")}${existingFiles.country_logo}`
                    }
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-amber-500"
                  >
                    {existingFiles.country_logo.split("/").pop()}
                  </a>
                )}
              </div>
            </div>
