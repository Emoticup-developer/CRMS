...
import { createLanguage } from "../../../store/slices/languageSlice";
...
const CreateLanguage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await dispatch(createLanguage(submitData)).unwrap();
      if (res.status === 200 || res.status === 201) {
        showTemporaryMessage("Language created successfully!", "success");
      } else if (res.status === 202) {
        showTemporaryMessage("Language create accepted!", "success");
      } else {
      ...
      if (errorMessages.length > 0) {
        ...
        showTemporaryMessage("Failed to create Language!", "error");
      }
    }
