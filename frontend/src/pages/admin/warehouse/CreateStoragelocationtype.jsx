...
const CreateStoragelocationtype = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await dispatch(
        createStoragelocationtype(submitData),
      ).unwrap();

      if (res.status === 200 || res.status === 201) {
        showTemporaryMessage(
          "Storagelocationtype created successfully!",
          "success",
        );
      } else {
      ...
      showTemporaryMessage("Failed to create Storagelocationtype!", "error");
    }
...
export default CreateStoragelocationtype;
