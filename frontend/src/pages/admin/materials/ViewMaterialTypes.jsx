...
import {
  ...
  fetchUsers,
  fetchCompanies,
} from "../../../store/slices/materialTypeSlice";
import { FaTimes, FaTrashAlt } from "react-icons/fa";
import dayjs from "dayjs";

const ViewMaterialTypes = () => {
  const dispatch = useDispatch();
  const { materialCategories, users, companies, materialTypes, loading } =
    useSelector((state) => state.materialTypes);
  ...
  const updateBreadcrumbs = (action = null, id = null) => {
    ...
    if (action === "Delete") {
      ...
      if (isBulkDelete && selectedMaterialTypes.length > 0) {
        baseBreadcrumbs.push({
          label: formatIdsWithEllipsis(selectedMaterialTypes),
          fullLabel: selectedMaterialTypes.join(", "),
          path: null,
        });
      } else if (deleteId) {
        baseBreadcrumbs.push({
  ...
  useEffect(() => {
    ...
    } else if (showConfirm && selectedMaterialType?.id) {
      updateBreadcrumbs("View", selectedMaterialType.id);
    } else {
      updateBreadcrumbs();
    }
  }, [
  ...
  const handleOpenEdit = (mttps) => {
    setEditId(mttps.id);
    setFormData({
      name: mttps.name || "",
      code: mttps.code || "",
      short_name: mttps.short_name || "",
      material_category: mttps.material_category?.id || "",
      description: mttps.description || "",
    });
  ...
  const handleSubmitEdit = async (e) => {
    ...
    try {
      const res = await dispatch(
        updateMaterialType({ id: editId, formData: data }),
      ).unwrap();

      if (res.status === 200 || res.status === 201) {
        showTemporaryMessage("Material Type updated successfully!", "success");
      } else if (res.status === 202) {
        showTemporaryMessage("Material Type update accepted!", "success");
      } else {
      ...
      if (errorMessages.length > 0) {
        ...
        showTemporaryMessage("Failed to update material type!", "error");
      }
    }
  };
  ...
  const confirmDelete = async () => {
    try {
      if (isBulkDelete && selectedMaterialTypes.length > 0) {
        const res = await Promise.all(
          selectedMaterialTypes.map((id) =>
            dispatch(deleteMaterialType(id)).unwrap(),
          ),
        );
        if (res.every((r) => r.status === 200 || r.status === 201)) {
          ...
          showTemporaryMessage("Material Type delete accepted!", "success");
        } else {
          showTemporaryMessage("Unexpected response from server.", "error");
          return;
        }
        ...
        const res = await dispatch(deleteMaterialType(deleteId)).unwrap();
        if (res.status === 200 || res.status === 201) {
          showTemporaryMessage(
            "Material Type deleted successfully!",
            "success",
      ...
      if (errorMessages.length > 0) {
        ...
        showTemporaryMessage("Failed to delete material group!", "error");
      }
    }
  };

  // ---------------- FILTER LOGIC ----------------
  const filteredMaterialTypes = materialTypes.filter((mttps) => {
    const search = searchTerm.toLowerCase().trim().replace(/\s+/g, " ");
    const normalize = (value) =>
      (value || "").toString().toLowerCase().trim().replace(/\s+/g, " ");

    return (
      normalize(mttps?.code).includes(search) ||
      normalize(mttps?.name).includes(search) ||
      normalize(mttps?.material_category?.name).includes(search) ||
      normalize(mttps?.creator?.username).includes(search) ||
      normalize(mttps?.company?.company_name).includes(search)
    );
  });

  // ---------------- PAGINATION LOGIC ----------------
  const totalPages = Math.ceil(filteredMaterialTypes.length / rowsPerPage);

  const paginatedMaterialTypes = filteredMaterialTypes.slice(
  ...
  return (
    <div className="flex h-screen">
              ...
              <table className="min-w-full text-sm text-left divide-y divide-gray-200">
                <thead className="bg-gray-100 text-gray-700 sticky top-0 z-10">
                  <tr>
                    <th className="px-2 py-2 border border-gray-200 text-center sticky top-0 z-20">
                      {filteredMaterialTypes.length <= 1 ? (
                        "-"
                      ) : (
                        <input
                          type="checkbox"
                          ...
                          onChange={(e) =>
                            setSelectedMaterialTypes(
                              e.target.checked
                                ? filteredMaterialTypes.map((mttps) => mttps.id)
                                : [],
                            )
                          }
                ...
                <tbody className="divide-y divide-gray-100 text-gray-800">
                  {loading ? (
                    ...
                    paginatedMaterialTypes.map((mttps) => (
                      <tr
                        key={mttps.id}
                      ...
                      >
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          <input
                            type="checkbox"
                            checked={selectedMaterialTypes.includes(mttps.id)}
                            onChange={() =>
                              setSelectedMaterialTypes((prev) =>
                                prev.includes(mttps.id)
                                  ? prev.filter((x) => x !== mttps.id)
                                  : [...prev, mttps.id],
                              )
                            }
                            className="w-4 h-4 cursor-pointer transition-all duration-200 ease-in-out transform hover:scale-110 active:scale-95 accent-amber-400"
                        ...
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          {mttps?.material_category?.name || "--"}
                        </td>
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          {mttps?.creator?.username || "--"}
                        </td>
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          {mttps?.company?.company_name || "--"}
                        </td>
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          <div className="flex justify-center items-center space-x-3 text-sm">
                            <button
                              onClick={() => handleOpenEdit(mttps)}
                            ...
                            <button
                              onClick={() => {
                                setSelectedMaterialType(mttps);
                                updateBreadcrumbs("View", mttps.id);
                                setShowConfirm(true);
                              }}
      ...
      {showConfirm && selectedMaterialType && (
              ...
              <table className="table-auto w-full text-sm text-gray-700">
                <tbody>
                  ...
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedMaterialType?.short_name || "--"}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    <td className="font-semibold w-2/5 py-1 text-left">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedMaterialType?.material_category?.name || "--"}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                  ...
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedMaterialType?.creator?.username || "--"}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    <td className="font-semibold w-2/5 py-1 text-left">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedMaterialType?.company?.company_name || "--"}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    <td className="font-semibold w-2/5 py-1 text-left">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedMaterialType?.created_at
                        ? dayjs(selectedMaterialType?.created_at).format(
                            "DD-MM-YYYY hh:mm A",
                          )
                        : "--"}
                    </td>
                  ...
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedMaterialType?.updated_at
                        ? dayjs(selectedMaterialType?.updated_at).format(
                            "DD-MM-YYYY hh:mm A",
                          )
                        : "--"}
                    </td>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-gray-700 max-h-[200px] overflow-y-auto [&::-webkit-scrollbar]:hidden scrollbar-none">
              ...
              <div className="flex flex-col">
                ...
                >
                  ...
                  {materialCategories.map((mtcty) => (
                    <option key={mtcty.id} value={mtcty.id}>
                      {mtcty.name}
                    </option>
                  ))}
                </select>
          ...
          <div className="bg-white p-4 rounded-md w-11/12 max-w-md shadow-xl">
            ...
            <div className="flex justify-center gap-4 mt-4">
              ...
              <button
                onClick={() => {
                  setShowDeleteModal(false);
                  if (isBulkDelete) setSelectedMaterialTypes([]);
                  setIsBulkDelete(false);
                  setDeleteId(null);
                }}
...
export default ViewMaterialTypes;
