...
import {
  ...
  fetchMaterialGroups,
} from "../../../store/slices/materialCategorySlice";
import { FaTimes, FaTrashAlt } from "react-icons/fa";
import dayjs from "dayjs";

const ViewMaterialCategories = () => {
  const dispatch = useDispatch();
  const { materialGroups, materialCategories, loading } = useSelector(
  ...
  const updateBreadcrumbs = (action = null, id = null) => {
    ...
    if (action === "Delete") {
      ...
      if (isBulkDelete && selectedMaterialCategories.length > 0) {
        baseBreadcrumbs.push({
          label: formatIdsWithEllipsis(selectedMaterialCategories),
          fullLabel: selectedMaterialCategories.join(", "),
          path: null,
        });
      } else if (deleteId) {
        baseBreadcrumbs.push({
  ...
  useEffect(() => {
    ...
    } else if (showConfirm && selectedMaterialCategory?.id) {
      updateBreadcrumbs("View", selectedMaterialCategory.id);
    } else {
      updateBreadcrumbs();
    }
  }, [
  ...
  const handleOpenEdit = (mtcty) => {
    setEditId(mtcty.id);
    setFormData({
      name: mtcty.name || "",
      code: mtcty.code || "",
      short_name: mtcty.short_name || "",
      material_group: mtcty.material_group?.id || "",
      description: mtcty.description || "",
    });
  ...
  const handleSubmitEdit = async (e) => {
    ...
    try {
      const res = await dispatch(
        updateMaterialCategory({ id: editId, formData: data }),
      ).unwrap();
      ...
      if (res.status === 200 || res.status === 201) {
        ...
        showTemporaryMessage("Material Category update accepted!", "success");
      } else {
      ...
      if (errorMessages.length > 0) {
        ...
        showTemporaryMessage("Failed to update material category!", "error");
      }
    }
  };
  ...
  const handleBulkDeleteClick = () => {
    if (selectedMaterialCategories.length === 0) return;
    ...
    try {
      if (isBulkDelete && selectedMaterialCategories.length > 0) {
        const res = await Promise.all(
          selectedMaterialCategories.map((id) =>
            dispatch(deleteMaterialCategory(id)).unwrap(),
          ),
        );
        if (res.every((r) => r.status === 200 || r.status === 201)) {
          ...
          showTemporaryMessage("Material Category delete accepted!", "success");
        } else {
          showTemporaryMessage("Unexpected response from server.", "error");
          return;
        }
        ...
        const res = await dispatch(deleteMaterialCategory(deleteId)).unwrap();
        if (res.status === 200 || res.status === 201) {
          showTemporaryMessage(
            "Material Category deleted successfully!",
            "success",
      ...
      if (errorMessages.length > 0) {
        ...
        showTemporaryMessage("Failed to delete material category!", "error");
      }
    }
  };

  // ---------------- FILTER LOGIC ----------------
  const filteredMaterialCategories = materialCategories.filter((mtcty) => {
    const search = searchTerm.toLowerCase().trim().replace(/\s+/g, " ");
    const normalize = (value) =>
      (value || "").toString().toLowerCase().trim().replace(/\s+/g, " ");

    return (
      normalize(mtcty?.code).includes(search) ||
      normalize(mtcty?.name).includes(search) ||
      normalize(mtcty?.material_group?.name).includes(search) ||
      normalize(mtcty?.creator?.username).includes(search) ||
      normalize(mtcty?.company?.company_name).includes(search)
    );
  });

  // ---------------- PAGINATION LOGIC ----------------
  const totalPages = Math.ceil(filteredMaterialCategories.length / rowsPerPage);

  const paginatedMaterialCategories = filteredMaterialCategories.slice(
  ...
  return (
    <div className="flex h-screen">
        ...
        <div className="w-full mb-4">
          <div className="flex justify-between items-end border-b-2 border-gray-300 pb-1 mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <Link
                to="/admin/material-category/create"
              ...
              >
                <FiPlus /> Create Material Category
              </Link>

              {selectedMaterialCategories.length > 1 && (
                <button
              ...
              <table className="min-w-full text-sm text-left divide-y divide-gray-200">
                <thead className="bg-gray-100 text-gray-700 sticky top-0 z-10">
                  <tr>
                    <th className="px-2 py-2 border border-gray-200 text-center sticky top-0 z-20">
                      {filteredMaterialCategories.length <= 1 ? (
                        "-"
                      ) : (
                        <input
                          type="checkbox"
                          checked={
                            filteredMaterialCategories.length > 1 &&
                            selectedMaterialCategories.length ===
                              filteredMaterialCategories.length
                          }
                          onChange={(e) =>
                            setSelectedMaterialCategories(
                              e.target.checked
                                ? filteredMaterialCategories.map(
                                    (mtcty) => mtcty.id,
                                  )
                                : [],
                            )
                ...
                <tbody className="divide-y divide-gray-100 text-gray-800">
                  {loading ? (
                    ...
                    paginatedMaterialCategories.map((mtcty) => (
                      <tr
                        key={mtcty.id}
                      ...
                      >
                          ...
                          <input
                            ...
                            onChange={() =>
                              setSelectedMaterialCategories((prev) =>
                                prev.includes(mtcty.id)
                                  ? prev.filter((x) => x !== mtcty.id)
                                  : [...prev, mtcty.id],
                              )
                            }
                            className="w-4 h-4 cursor-pointer transition-all duration-200 ease-in-out transform hover:scale-110 active:scale-95 accent-amber-400"
                        ...
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          {mtcty?.material_group?.name || "--"}
                        </td>
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          {mtcty?.creator?.username || "--"}
                        </td>
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          {mtcty?.company?.company_name || "--"}
                        </td>
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          <div className="flex justify-center items-center space-x-3 text-sm">
                            <button
                              onClick={() => handleOpenEdit(mtcty)}
                            ...
                            <button
                              onClick={() => {
                                setSelectedMaterialCategory(mtcty);
                                updateBreadcrumbs("View", mtcty.id);
                                setShowConfirm(true);
                              }}
      ...
      {showConfirm && selectedMaterialCategory && (
              ...
              <table className="table-auto w-full text-sm text-gray-700">
                <tbody>
                  ...
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedMaterialCategory?.short_name || "--"}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    <td className="font-semibold w-2/5 py-1 text-left">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedMaterialCategory?.material_group?.name || "--"}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4 align-top">
                      {selectedMaterialCategory?.description || "--"}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    <td className="font-semibold w-2/5 py-1 text-left">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedMaterialCategory?.creator?.username || "--"}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    <td className="font-semibold w-2/5 py-1 text-left">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedMaterialCategory?.company?.company_name || "--"}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    <td className="font-semibold w-2/5 py-1 text-left">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedMaterialCategory?.created_at
                        ? dayjs(selectedMaterialCategory?.created_at).format(
                            "DD-MM-YYYY hh:mm A",
                          )
                        : "--"}
                    </td>
                  ...
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedMaterialCategory?.updated_at
                        ? dayjs(selectedMaterialCategory?.updated_at).format(
                            "DD-MM-YYYY hh:mm A",
                          )
                        : "--"}
                    </td>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-gray-700 max-h-[200px] overflow-y-auto [&::-webkit-scrollbar]:hidden scrollbar-none">
              ...
              <div className="flex flex-col">
                ...
                >
                  ...
                  {materialGroups.map((mtgrp) => (
                    <option key={mtgrp.id} value={mtgrp.id}>
                      {mtgrp.name}
                    </option>
                  ))}
                </select>
          ...
          <div className="bg-white p-4 rounded-md w-11/12 max-w-md shadow-xl">
            ...
            <div className="flex justify-center gap-4 mt-4">
              ...
              <button
                onClick={() => {
                  setShowDeleteModal(false);
                  if (isBulkDelete) setSelectedMaterialCategories([]);
                  setIsBulkDelete(false);
                  setDeleteId(null);
                }}
...
export default ViewMaterialCategories;
