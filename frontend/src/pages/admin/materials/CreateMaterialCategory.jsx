...
const CreateMaterialCategory = () => {
  ...
  const { materialGroups } = useSelector((state) => state.materialCategories);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await dispatch(createMaterialCategory(submitData)).unwrap();
      if (res.status === 200 || res.status === 201) {
        ...
        showTemporaryMessage("Material Category create accepted!", "success");
      } else {
      ...
      if (errorMessages.length > 0) {
        ...
        showTemporaryMessage("Failed to create material category!", "error");
      }
    }
