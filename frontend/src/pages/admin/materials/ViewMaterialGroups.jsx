...
const ViewMaterialGroups = () => {
  const dispatch = useDispatch();
  const { materialGroups, loading } = useSelector(
    (state) => state.materialGroups,
  );
  ...
  const [selectedMaterialGroups, setSelectedMaterialGroups] = useState([]);
  ...
  const updateBreadcrumbs = (action = null, id = null) => {
    ...
    if (action === "Delete") {
      ...
      if (isBulkDelete && selectedMaterialGroups.length > 0) {
        baseBreadcrumbs.push({
          label: formatIdsWithEllipsis(selectedMaterialGroups),
          fullLabel: selectedMaterialGroups.join(", "),
          path: null,
        });
      } else if (deleteId) {
        baseBreadcrumbs.push({
  ...
  useEffect(() => {
    ...
    } else if (showConfirm && selectedMaterialGroup?.id) {
      updateBreadcrumbs("View", selectedMaterialGroup.id);
    } else {
      updateBreadcrumbs();
    }
  }, [
  ...
  const handleOpenEdit = (mtgrp) => {
    setEditId(mtgrp.id);
    setFormData({
      name: mtgrp.name || "",
      code: mtgrp.code || "",
      short_name: mtgrp.short_name || "",
      description: mtgrp.description || "",
    });
  ...
  const handleSubmitEdit = async (e) => {
    ...
    try {
      const res = await dispatch(
        updateMaterialGroup({ id: editId, formData: data }),
      ).unwrap();

      if (res.status === 200 || res.status === 201) {
        showTemporaryMessage("Material Group updated successfully!", "success");
      } else if (res.status === 202) {
        showTemporaryMessage("Material Group update accepted!", "success");
      } else {
      ...
      if (errorMessages.length > 0) {
        ...
        showTemporaryMessage("Failed to update material group!", "error");
      }
    }
  };
  ...
  const handleBulkDeleteClick = () => {
    if (selectedMaterialGroups.length === 0) return;
    ...
    try {
      if (isBulkDelete && selectedMaterialGroups.length > 0) {
        const res = await Promise.all(
          selectedMaterialGroups.map((id) =>
            dispatch(deleteMaterialGroup(id)).unwrap(),
          ),
        );
        if (res.every((r) => r.status === 200 || r.status === 201)) {
          ...
          showTemporaryMessage("Material Group delete accepted!", "success");
        } else {
          showTemporaryMessage("Unexpected response from server.", "error");
          return;
        }
        ...
        const res = await dispatch(deleteMaterialGroup(deleteId)).unwrap();
        if (res.status === 200 || res.status === 201) {
          showTemporaryMessage(
            "Material Group deleted successfully!",
            "success",
  ...
  // ---------------- FILTER LOGIC ----------------
  const filteredMaterialGroups = materialGroups.filter((mtgrp) => {
    const search = searchTerm.toLowerCase().trim().replace(/\s+/g, " ");
    const normalize = (value) =>
      (value || "").toString().toLowerCase().trim().replace(/\s+/g, " ");

    return (
      normalize(mtgrp?.code).includes(search) ||
      normalize(mtgrp?.name).includes(search) ||
      normalize(mtgrp?.short_name).includes(search) ||
      normalize(mtgrp?.creator?.username).includes(search) ||
      normalize(mtgrp?.company?.company_name).includes(search)
    );
  });

  // ---------------- PAGINATION LOGIC ----------------
  const totalPages = Math.ceil(filteredMaterialGroups.length / rowsPerPage);

  const paginatedMaterialGroups = filteredMaterialGroups.slice(
  ...
  return (
    <div className="flex h-screen">
        ...
        <div className="w-full mb-4">
          <div className="flex justify-between items-end border-b-2 border-gray-300 pb-1 mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <Link
                to="/admin/material-group/create"
              ...
              >
                <FiPlus /> Create Material Group
              </Link>

              {selectedMaterialGroups.length > 1 && (
                <button
              ...
              <table className="min-w-full text-sm text-left divide-y divide-gray-200">
                <thead className="bg-gray-100 text-gray-700 sticky top-0 z-10">
                  <tr>
                    <th className="px-2 py-2 border border-gray-200 text-center sticky top-0 z-20">
                      {filteredMaterialGroups.length <= 1 ? (
                        "-"
                      ) : (
                        <input
                          type="checkbox"
                          checked={
                            filteredMaterialGroups.length > 1 &&
                            selectedMaterialGroups.length ===
                              filteredMaterialGroups.length
                          }
                          onChange={(e) =>
                            setSelectedMaterialGroups(
                              e.target.checked
                                ? filteredMaterialGroups.map(
                                    (mtgrp) => mtgrp.id,
                                  )
                                : [],
                            )
                ...
                <tbody className="divide-y divide-gray-100 text-gray-800">
                  {loading ? (
                    ...
                    paginatedMaterialGroups.map((mtgrp) => (
                      <tr
                        key={mtgrp.id}
                      ...
                      >
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          <input
                            type="checkbox"
                            checked={selectedMaterialGroups.includes(mtgrp.id)}
                            onChange={() =>
                              setSelectedMaterialGroups((prev) =>
                                prev.includes(mtgrp.id)
                                  ? prev.filter((x) => x !== mtgrp.id)
                                  : [...prev, mtgrp.id],
                              )
                            }
                            className="w-4 h-4 cursor-pointer transition-all duration-200 ease-in-out transform hover:scale-110 active:scale-95 accent-amber-400"
                        ...
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          {mtgrp?.creator?.username || "--"}
                        </td>
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          {mtgrp?.company?.company_name || "--"}
                        </td>
                        <td className="px-2 py-2 border border-gray-200 whitespace-nowrap">
                          <div className="flex justify-center items-center space-x-3 text-sm">
                            <button
                              onClick={() => handleOpenEdit(mtgrp)}
                            ...
                            <button
                              onClick={() => {
                                setSelectedMaterialGroup(mtgrp);
                                updateBreadcrumbs("View", mtgrp.id);
                                setShowConfirm(true);
                              }}
      ...
      {showConfirm && selectedMaterialGroup && (
              ...
              <table className="table-auto w-full text-sm text-gray-700">
                <tbody>
                  ...
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedMaterialGroup?.short_name || "--"}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4 align-top">
                      {selectedMaterialGroup?.description || "--"}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    <td className="font-semibold w-2/5 py-1 text-left">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedMaterialGroup?.creator?.username || "--"}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    <td className="font-semibold w-2/5 py-1 text-left">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedMaterialGroup?.company?.company_name || "--"}
                    </td>
                  </tr>
                  <tr className="border-b border-gray-200">
                    <td className="font-semibold w-2/5 py-1 text-left">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedMaterialGroup?.created_at
                        ? dayjs(selectedMaterialGroup?.created_at).format(
                            "DD-MM-YYYY hh:mm A",
                          )
                        : "--"}
                    </td>
                  ...
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="w-3/5 py-1 text-left pl-4">
                      {selectedMaterialGroup?.updated_at
                        ? dayjs(selectedMaterialGroup?.updated_at).format(
                            "DD-MM-YYYY hh:mm A",
                          )
                        : "--"}
                    </td>
          ...
          <div className="bg-white p-4 rounded-md w-11/12 max-w-md shadow-xl">
            ...
            <div className="flex justify-center gap-4 mt-4">
              ...
              <button
                onClick={() => {
                  setShowDeleteModal(false);
                  if (isBulkDelete) setSelectedMaterialGroups([]);
                  setIsBulkDelete(false);
                  setDeleteId(null);
                }}
...
export default ViewMaterialGroups;
