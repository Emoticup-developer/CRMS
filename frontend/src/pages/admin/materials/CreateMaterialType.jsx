...
import { FiInfo, FiSave, FiTool } from "react-icons/fi";
import {
  createMaterialType,
...
const CreateMaterialType = () => {
  ...
  const { materialCategories } = useSelector((state) => state.materialTypes);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await dispatch(createMaterialType(submitData)).unwrap();
      if (res.status === 200 || res.status === 201) {
        showTemporaryMessage("Material Type created successfully!", "success");
      } else if (res.status === 202) {
        showTemporaryMessage("Material Type create accepted!", "success");
      } else {
      ...
      if (errorMessages.length > 0) {
        ...
        showTemporaryMessage("Failed to create material type!", "error");
      }
    }
